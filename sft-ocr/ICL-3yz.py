...
ds = load_dataset("./data/ICDAR2015", cache_dir="./cache")
data = ds["test"]


train_data = ds["train"]
icl_examples = [train_data[i] for i in range(5)]
...
def calculate_accuracy(test_data):
    ...
    for idx, sample in enumerate(test_data):
        ...
        for ex in icl_examples:
            messages.extend([
                {"role": "user", "content": [{"type": "image", "image": ex["image"]}, {"type": "text", "text": "识别图片中的文字"}]},
                {"role": "assistant", "content": [{"type": "text", "text": ex["text"]}]}
            ])
        messages.append({
            "role": "user",
            "content": [{"type": "image", "image": sample["image"]}, {"type": "text", "text": "识别图片中的文字"}],
        })
    ...
    avg_edit_distance = total_samples and total_edit_distance / total_samples or 0
